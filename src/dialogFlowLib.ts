...
import * as gitpleBot from 'gitple-bot';
...
export function init(options: any, cb?: (err?: Error) => void) {
...
botMgr.on('start', (botConfig, done) => {
  let gpBot: gitpleBot.Bot = new gitpleBot.Bot(botMgr, botConfig, {
    dialogFlowSessionId: uuid.v4(), // state
  });

  if (!gpBot || !botConfig || !botConfig.context) { return; }
...
async function startChat(gpBot: any) { // jshint ignore:line
  ...
  const request = {
    session: sessionPath,
    queryInput: {
      event: { name: 'WELCOME', languageCode: 'ko', }, // WELCOME event
    },
  };
...
function handleBotMessage(inputMessage: any) {
